...
namespace UIDE {
	...
	public class UIDEPlugin:System.Object {
		...
		public virtual void OnClickMoveCursor(Vector2 pos) {
			
		}
		public virtual void OnDragMoveCursor(Vector2 pos) {
			
		}
		public virtual void OnArrowKeyMoveCursor(Vector2 pos) {
			
		}
		public virtual void OnArrowKeyMoveCursorLine(Vector2 pos) {
		...
		static public UIDEPlugin[] GetPluginInstances(UIDETextEditor textEditor) {
			...
			foreach (Type type in UIDEEditor.current.pluginSettings.GetPluginTypes()) {
				if (!UIDEEditor.current.pluginSettings.IsPluginEnabled(type)) continue;
				types.Add(type);
			...
			*/
			...
			for (int i = 0; i < types.Count; i++) {
				...
				if (instance.excludeFromLoading) continue;
				if (!instance.CheckIfShouldLoad(textEditor)) continue;
