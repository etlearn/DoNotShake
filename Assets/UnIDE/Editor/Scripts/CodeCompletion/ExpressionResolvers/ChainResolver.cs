...
namespace UIDE.CodeCompletion {
	...
	public class ChainResolver:System.Object {
		...
		public void Refresh(UIDETextEditor editor, Vector2 cursorPos) {
			this.editor = editor;
			this.cursorPos = cursorPos;
			reflectionDB.editor = editor;
			reflectionDB.cursorPos = cursorPos;
			reflectionDB.UpdateCurrentState(false);
		}
		...
		public void ResolveCurrentlyVisibleCompletionItem(CompletionItem member) {
			...
			if (member.resultingType == null) {
				...
				if (t == null && member.inferredType) {
					...
					if (resolvedExpression != member.name) {
				...
				if (t == null && member.isVariable && member.variableDeclarationBlock != "") {
					bool isNotDum = member.variableDeclarationBlock != member.name;
					if (isNotDum && member.variableDeclarationBlock.StartsWith(member.name+".")) {
						isNotDum = false;
					}
					//Debug.Log(member.variableDeclarationBlock+" "+member.name+" "+isNotDum);
		...
		private CompletionItem GetVariableFromVisible(ChainItem item) {
			...
			for (int i = 0; i < reflectionDB.currentCompletionItems.Length; i++) {
				//Debug.Log(reflectionDB.currentCompletionItems[i].name);
