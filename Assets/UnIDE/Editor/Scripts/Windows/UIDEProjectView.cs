...
public enum UIDEProjectItemType {Other,ScriptCS,ScriptJS,ScriptBoo,Shader};
...
public class UIDEProjectItem:System.Object {
	...
	public void UpdateInfo() {
		...
		if (!isFolder) {
			...
			type = UIDEProjectItemType.Other;
			if (extension == "cs") {
...
public class UIDEProjectView:UIDEWindow {
	...
	public void UpdateDB() {
		...
		if (selectedItem != null && selectedItem.path != null) {
	...
	UIDEProjectItem CreateItem(string path, bool isFolder) {
		...
		if (isFolder) {
			...
			if (editorWindow != null && editorWindow.generalSettings != null) {
				string[] supportedTypes = editorWindow.generalSettings.GetSupportedFileTypes();
				for (int i = 0; i < supportedTypes.Length; i++) {
					files.AddRange(GetFilesOfTypeInDirectory(path,"*."+supportedTypes[i]));
				}
			}
	...
	public void UpdateRects() {
		sepSliderValue = rect.height*0.3f;
	...
	void DrawFileView() {
		...
		for (int i = start; i < end; i++) {
			...
			GUIContent buttonContent = new GUIContent(itemList[i].nameNoExt);
			Vector2 desiredSize = buttonStyle.CalcSize(buttonContent);
			if (r.width < desiredSize.x) {
				buttonContent.tooltip = itemList[i].name;
			}
			if (GUI.Button(r,buttonContent,buttonStyle)) {
	...
	void DrawFileViewToolbar(Rect toolbarRect) {
		...
		if (!isCollapsed) {
			...
			//if (GUILayout.Button("Subfiles",EditorStyles.toolbarButton)) {
	...
	int DrawProjectItem(UIDEProjectItem item, int yPos, int indent) {
		...
		if (!clickedExpander && UIDEGUI.TestClick(0,invisButtonRect)||UIDEGUI.TestClick(1,invisButtonRect)) {
			selectedItem = item;
