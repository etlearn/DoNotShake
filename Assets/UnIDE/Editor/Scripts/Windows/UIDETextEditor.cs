...
public class UIDETextEditor:UIDEWindow {
	...
	private void TouchDummyAsset() {
		...
		Undo.RegisterSceneUndo("UnIDE Focus");
		Undo.PerformUndo();
		Undo.PerformRedo();
		...
		*/
		...
		isFocusDummyUndoRedo = true;
		Texture2D dummyTex = (Texture2D)UIDEEditor.LoadAsset("Assets/UnIDE/Editor/DUMMYASSET.psd");
		
		TextureImporter textureImporter = (TextureImporter)AssetImporter.GetAtPath("Assets/UnIDE/Editor/DUMMYASSET.psd"); 
		textureImporter.mipmapEnabled = !textureImporter.mipmapEnabled;
		//textureImporter.mipmapEnabled = false;
		AssetDatabase.ImportAsset("Assets/UnIDE/Editor/DUMMYASSET.psd"); 
		...
		//Undo.RegisterSceneUndo("penises");
		/*
		Texture2D dummyTex = (Texture2D)UIDEEditor.LoadAsset("Assets/UnIDE/Editor/DUMMYASSET.psd");
		UnityEngine.Object orig = Selection.activeObject;
		Undo.RegisterSceneUndo("buttys");
		Selection.activeObject = dummyTex;
		//Undo.RegisterSceneUndo("vaginas");
		Selection.activeObject = null;
		Undo.RegisterSceneUndo("penises");
		Selection.activeObject = orig;
		//Undo.RegisterSceneUndo("boobs");
		Resources.UnloadAsset(dummyTex);
		Undo.RegisterSceneUndo("lools");
		*/
	}
	...
	public void OnGUI() {
		...
		//Catch typing input
		if (editorWindow.canTextEditorInteract && enableTextEnter) {
			...
			//mouseDown |= Event.current.type == EventType.MouseDrag;
			//mouseDown |= GUI.GetNameOfFocusedControl() == "";
			//if (mouseDown && textEditorNoScrollBarRectZeroPos.Contains(windowMousePos) && TestClickBlockers(windowMousePos)) {
				//Debug.Log("Focused "+Time.realtimeSinceStartup);
			...
			//GUI.SetNextControlName("UIDEGoBackToTextArea");
			//GUI.TextField(new Rect(0,-10000,0,0),"");
			//if (GUI.GetNameOfFocusedControl() == "UIDEGoBackToTextArea") {
			//	GUI.FocusControl("UIDETextAreaDummy");
			//}
		}
	}
