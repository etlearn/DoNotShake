...
namespace UIDE.SettingsMenu.Groups {
	public class SGText:SettingsGroup {
		static public string fontPath {
			get {
				return UIDEEditor.baseDir+"Editor/TextEditorFonts/";
			}
		}
		...
		public Font UpdateFont(bool forceUpdate) {
			...
			if (forceUpdate || font != data.font || boldFont != data.boldFont) {
				System.Type fontProcessorType = System.Type.GetType("UIDE.UIDEFontProcessor");
				if (fontProcessorType == null) {
					fontProcessorType = System.Type.GetType("UIDE.UIDEFontProcessor,Assembly-CSharp-Editor");
				}
				MethodInfo processMethod = fontProcessorType.GetMethod("Process");
				
				if (font != null) {
					processMethod.Invoke(null,new System.Object[] {currentFullFontPath,data.fontSize,data.forceDynamicFont35});
				}
				if (boldFont != null) {
					processMethod.Invoke(null,new System.Object[] {boldPath,data.fontSize,data.forceDynamicFont35});
				}
			}
			data.font = font;
